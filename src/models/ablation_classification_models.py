...
def create_lstm_classifier(input_shape, num_classes):
    ...
    # First LSTM layer (returns full sequence to feed the next one)
...
def train_ablation_cv_classifier(encoded_all, X_all_original, Y_all, scaler, epochs):
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(encoded_all)):
        print(f"\n--- Fold {fold + 1} ---")
